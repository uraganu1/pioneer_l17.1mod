...
static struct qpnp_hap *ghap = NULL;
...
{
	struct timed_output_dev *timed_dev = dev_get_drvdata(dev);
	struct qpnp_hap *hap = container_of(timed_dev, struct qpnp_hap, timed_dev);
        return snprintf(buf, PAGE_SIZE, "%d\n", hap->vmax_mv);
}
...
static struct device_attribute qpnp_hap_attrs[] = {
	...
	__ATTR(vtg_level, (S_IRUGO | S_IWUSR | S_IWGRP),
       ...
       __ATTR(vtg_min, S_IRUGO,
                qpnp_hap_min_show, NULL),
       __ATTR(vtg_max, S_IRUGO,
                qpnp_hap_max_show, NULL),
       __ATTR(vtg_default, S_IRUGO,
                qpnp_hap_default_show, NULL),
};
